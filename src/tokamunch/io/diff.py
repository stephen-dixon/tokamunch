...
def render_diff(
    ...
    *,
    ...
    from .outputs import format_value
    ...
    for entry in entries:
        if entry.status == "unchanged":
            if show_unchanged:
                lines.append(f"  {entry.path}: {format_value(entry.value_a)}")
        elif entry.status == "added":
            lines.append(f"+ {entry.path}: {format_value(entry.value_b)}")
        elif entry.status == "removed":
            lines.append(f"- {entry.path}: {format_value(entry.value_a)}")
        elif entry.status == "changed":
            lines.append(
                f"~ {entry.path}: {format_value(entry.value_a)} → {format_value(entry.value_b)}"
            )
...
def diff_files(
    ...
    *,
    ...
    """
    from ..ids.output import SUPPORTED_SUFFIXES
